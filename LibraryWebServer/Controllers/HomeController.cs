...
namespace LibraryWebServer.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult CheckLogin(string name, int cardnum)
        {
            ...
            {
                loginSuccessful = (from p in db.Patrons
        ...
        public ActionResult AllTitles()
        {
            using (Team75LibraryContext db = new Team75LibraryContext())
            {
                var query =
                    ...
                    into serials
                    from j in serials.DefaultIfEmpty()
                    join c in db.CheckedOut on j.Serial equals c.Serial
                    into checks
                    from p in checks.DefaultIfEmpty()
                    join m in db.Patrons on p.CardNum equals m.CardNum
                    into patrons
                    from l in patrons.DefaultIfEmpty()
                    select new
                    {
                        isbn = t.Isbn,
                        title = t.Title,
        ...
        public ActionResult ListMyBooks()
        {
            using (Team75LibraryContext db = new Team75LibraryContext())
            {
                var checkdOutBooks =
                    ...
                    join m in db.Inventory on c.Serial equals m.Serial
                    into inven
                    from g in inven
                    join d in db.Titles on g.Isbn equals d.Isbn
                    into books
                    from b in books
                    select new
                    {
                ...
                return Json(checkdOutBooks.ToArray());
            }
        }
        ...
        public ActionResult ReturnBook(int serial)
        {
            ...
            {
                var returnBook =
                    from b in db.CheckedOut
                    where b.Serial == serial
                    select b;
                db.CheckedOut.RemoveRange(returnBook);
                db.SaveChanges();
                return Json(new { success = true });
            }            
        }
